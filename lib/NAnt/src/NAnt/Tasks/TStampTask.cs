...
namespace SourceForge.NAnt.Tasks {
    ...
    public class TStampTask : Task {
        ...
        protected override void ExecuteTask() {
            ...
            Project.Properties["tstamp.date"] = now.ToString("yyyyMMdd");
            Project.Properties["tstamp.time"] = now.ToString("HHmm");
            Project.Properties["tstamp.now"] = now.ToString();

            // set custom property
            if (_property != null && _pattern != null)
            {
                Project.Properties[_property] = now.ToString(_pattern);
                Log.WriteLineIf(Verbose, LogPrefix + _property + " = " + Project.Properties[_property].ToString());
            }

            // set properties set in formatters nested elements
            foreach (FormatterElement f in _formatterElements) {
                Project.Properties[f.Property] = now.ToString(f.Pattern);
                Log.WriteLineIf(Verbose, LogPrefix + f.Property + " = " + Project.Properties[f.Property].ToString());
            }
